...
> **認証について:** Coreライブラリは、APIキーなしで公開エンドポイント（モデル、画像、タグ、クリエイター）をクエリできます。APIキーが必要なのは、お気に入り、非公開モデル、より高いレート制限などの認証機能のみです。これはCivitaiSharp.Sdkとは異なり、**すべての操作でAPIトークンが必須**です。
